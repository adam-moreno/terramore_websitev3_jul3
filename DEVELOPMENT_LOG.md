...
Each stage tile (`StageItem`) now has its own IntersectionObserver instead of one container-level trigger. New `.solutions-node-tease` CSS: unrevealed tiles sit at 22% opacity (a visible tease that more is coming) and lift to full strength as they scroll into view. Desktop keeps a small stagger via `(index % 3) * 120ms` delays. Reduced-motion shows everything immediately. Verified: on mobile load only 01 Strategy is lit; 02–05 reveal on scroll.

### /marketing hero rebuilt (Superside ad-creative translation)
- **Copy translated from superside.com/ad-creative:** H1 "On-brand, on-time marketing designed to perform." Sub: "Whether it's Google, Meta, TikTok, email, or your website, get the campaigns you need fast — planned, built, launched, and measured by one connected system." Service list under a rule: Marketing strategy · Ad creative · Paid campaigns · Landing pages · Email & SMS · Short-form content · Automation · Analytics & attribution. New "Ads lose effectiveness quickly…" band below the hero (their fatigue framing, our system answer). Positioned honestly — no portfolio/brand-count claims.
- **Hero visual (`components/marketing-visuals.tsx`, new):** AI-generated lifestyle photo (`public/marketing/hero-phone.png`, owner on phone, warm cream/navy tones) with an overlapping Terra IQ phone mockup showing site visits (1,284 +18% with spark bars), sales today ($3,420), and a Stripe payments feed. The feed auto-scrolls up and back on an 11s CSS loop (`marketing-phone-scroll`, off under reduced motion) — Hearth-style. Floating "Campaigns live" chip. All numbers are illustrative UI, marked aria-hidden.
- Rest of /marketing (system row, capabilities, who-it's-for, CTA) unchanged. `pnpm build` clean.

### Ops note
The overnight dev terminal was aborted but left an orphaned `next dev` on :3111 serving stale HMR chunks — pages served but never hydrated (no React fibers). Fixed by pkill + fresh `pnpm dev --port 3111`. Second incident: running `pnpm build` while dev was up corrupted `.next` (unstyled HTML). Rule: stop dev before building.

### /marketing sections 2–4 (Superside translation continued)
Right below the hero, in order:
1. **`ServiceCarousel`** (full-bleed): infinite marquee of 8 service tiles (Marketing strategy, Ad creative, Paid campaigns, Landing pages, Email & SMS, Short-form content, Automation, Analytics & attribution), each with a real photographic still (AI-generated in the brand palette, `public/marketing/services/*.png` — v2 differentiation pass per detailed brief — each tile is a different visual artifact/environment: strategy = top-down flat-lay of a customer-journey map w/ sticky notes (warm paper, no people); creative = colorful wall of pinned ad variants (bright studio); paid = neutral media-buying dashboard w/ budget/ROAS rows + calculator; landing = clean white browser+phone device comp w/ CTA; email = warm evening desk, SMS phone + email-sequence laptop; video = vivid creator studio, vertical phone + edit timeline; automation = full-frame dark workflow node canvas (no people); analytics = cool-blue close-up of sankey conversion paths + funnel. Color environments intentionally vary; site chrome carries the brand. Taxonomy check: no duplicate services in code — the repeated tiles the user saw are the marquee's aria-hidden duplicate copy used for seamless looping. v1 photos and original CSS mini-mockups superseded. rAF-driven: track translates at ~33px/s, eases to a crawl on hover (speed lerp), loops via modulo of half the duplicated track. Under reduced motion it's a plain swipeable rail. Mobile shows the same marquee full-bleed.
2. **Ad-fatigue section:** Superside layout — eyebrow "Fresh campaigns fuel performance" with underline rule, serif-italic lead "Tired of ad fatigue?" + "It's time to refresh your marketing.", their fatigue copy adapted, AI-generated couch photo right (`public/marketing/fatigue-couch.png`).
3. **`StatsCountUp`** ("The data is clear / Terramore is *your shortcut* to revenue."): three factual stats animate 0→final on scroll-in (ease-out cubic, 1.6s): 10+ years, F500 (Fortune 500 advertisers measured at Kantar/Samba TV), 100% (logins/data/assets stay yours). Reduced motion shows finals immediately.
Then the existing "How the system runs" and below continue unchanged. Verified: marquee cruise + hover crawl measured via transform deltas; count-up observed mid-flight (F241→F500).
...
## Marketing page: tile compression, placeholder stats, creative tiles – September 19, 2026

**Carousel tiles reordered + shrunk.** Service tiles in `ServiceCarousel` are now compact horizontal cards (~16–17.5rem wide): label + note on the left carry the attention, photo reduced to a 14–16px-rounded 56–64px square thumbnail on the right. Carousel behavior (rAF marquee, hover slowdown, reduced-motion) unchanged.

**Hero service list removed.** The static `HERO_SERVICES` list under the hero CTAs is gone; the carousel is now the only enumeration of services.

**Stats replaced with customer-facing placeholders.** Old credential stats (10+ years / F500 / 100%) replaced with outcome stats: 3.4x average ROAS, 7 days to first campaign live, +38% lift in booked calls in 90 days. `StatsCountUp` now supports decimals via `toFixed`. **These numbers are placeholder fakes (user-approved for now) and must be replaced with real client results before shipping as claims.**

**New "Creative that keeps up" section** after the stats: `CreativeTilesGrid` — four non-clickable animated tiles (Video production: timeline with sweeping playhead; Motion graphics: orbiting/bouncing/pulsing shapes; Illustration: self-drawing SVG line sketch; Transcreation & translation: two speech bubbles trading prominence). All animation is pure CSS (`marketing-*` keyframes in `globals.css`), disabled under `prefers-reduced-motion`. Note: `bg-gold` is not a valid Tailwind class in this config (gold has no DEFAULT) — use `bg-gold-from` / `bg-gold-to`; `.text-gold` is a custom gradient utility in `globals.css`.

**Update (same day): 8-tile bento + booking CTA card.** Creative section expanded from 4 to 8 tiles (added Copywriting, Social content, Landing pages, Static ad design), each with its own CSS animation (typing lines + caret, floating hearts on a vertical story phone, shimmer over a page skeleton, gently swaying fanned ad deck). Desktop (`lg`): 4-col bento grid (`auto-rows-[11rem]`) — two 2x2 squares, two 1x2 talls, four 1x1 squares = perfectly filled 4x4, verified via computed layout (504x368 / 244x368 / 244x176). Mobile: one column, all tiles uniform (350x208 at 390px). Labels moved to white blur pills overlaid inside each tile so every size works; big tiles also show the note. Below the grid: `CreativeCtaCard` — Superside-style full-width card over an AI-generated photo (`public/marketing/cta-pocket.png`, phone in a jean back pocket showing a colorful mobile ad), left-side ink gradient scrim, "Get ads people actually stop for." headline, primary "Book a call" → `/book`, secondary "See all services" → `/solutions`. All new animations disabled under `prefers-reduced-motion`.

**Update (same day): CollaborationSteps section.** New `/marketing` section between the creative bento grid and the booking CTA card: sticky headline on the left ("A partner in the work, start to finish."), five steps on the right — Brainstorm, Creative back-and-forth, Fast deliverables, Implementation, Testing while live. An IntersectionObserver with `rootMargin: -40% 0px -50%` marks the step crossing the viewport's middle band as active: active step gets a white card + solid brand number, completed steps get a light-brand number, upcoming steps are dimmed to 45% opacity along a vertical progress rail. The CreativeCtaCard moved from the creative section to the end of this section so the booking ask follows the process story. Verified in-browser that the highlight advances and recedes with scroll.

**Update (same day): mid-page CTA, related articles, FAQ, cartoon footer band, "Book a demo".** /marketing changes: (1) All page-level CTAs (hero button, mid-page tile, jean-pocket card, FAQ footer link, cartoon band) now say "Book a demo" → `/book`; the header menu keeps "Let's talk" per request. (2) New dark-gradient CTA tile ("Never let a campaign go stale again.") between the ad-fatigue section and the count-up stats. (3) New "Related articles" section: three placeholder article cards (gradient thumbs, tag pill, read time) — intentionally not linked; wire real hrefs when the blog exists. (4) New extensive FAQ (`#faq`, 12 Q&As) built with native `<details>/<summary>` accordions — zero JS, rotating + badge on open. Note: site footer's "Questions" link points to `/#faq` (homepage), unaffected. (5) The closing "Ready to see the next step?" section was replaced by a cartoon footer band: AI-generated original Adventure-Time-adjacent landscape (`public/marketing/footer-cartoon.png`, no existing IP), with a frosted white card ("Your next campaign is an adventure away.") and Book a demo button, sitting directly above the standard `SiteFooter`. The old credential stats were previously replaced with placeholder outcome stats — still pending real numbers.

**Update (same day): mobile hero CTA, section pruning, real articles, cartoon footer.** (1) Hero "Book a demo" is full-width on mobile (`w-full sm:w-auto`, container `items-stretch`), with "Or get a free Digital Footprint report" centered underneath on mobile (`text-center sm:text-left`); verified 350px button = container width at 390px viewport. (2) Removed three sections from /marketing: "How the system runs", "What we work on", "Who it is for" (and their SYSTEM/CAPABILITIES constants). (3) Wrote three real article pages under `app/articles/`: `why-your-best-ad-stops-working`, `three-numbers-before-raising-ad-spend`, `your-landing-page-is-the-problem` — each with metadata, an AI-generated stock-style hero image (`public/marketing/articles/*.png`), full prose, a Book a demo CTA card, and back-link; all return 200. Related-article cards on /marketing now use those images and link to the pages. (4) Removed the standalone "adventure away" CTA band. `SiteFooter` now accepts optional `tagline` and `backgroundImage` props (defaults unchanged for all other pages); /marketing passes tagline "Your next campaign is an adventure away." and the cartoon image, drawn behind a white gradient wash (85%→40% top-to-bottom) so links stay legible while the artwork shows at the bottom.

**Update (same day): Superside-style immersive footer.** `SiteFooter` with `backgroundImage` now renders a distinct dark variant modeled on Superside's footer: cartoon artwork full-bleed under an ink wash (ink/70 + top/bottom gradient), big centered headline mixing sans + serif italic ("Your next campaign is *an adventure away.*") with a white pill "Book a demo" button; two labeled column groups ("Services", "Navigation") each with a hairline rule under the label and two bold-headed sub-columns (Creative & campaigns / Systems & data; Explore / Company) with links in white/60 → white on hover; bottom bar with white logo (`on="dark"`), copyright, inline legal links + Do-not-sell button, and the results disclaimer in white/40. The default light footer is untouched for all other pages.

**Update (same day): footer character animations.** Two AI-generated cartoon sprites now roam the immersive /marketing footer: a fire-breathing dragon (`public/marketing/footer-dragon.png`) flying left→right at ~24% height with a 2.4s vertical bob (38s crossing loop), and a chase scene (`public/marketing/footer-chase.png` — panicked runner fleeing a grinning monster truck hauling a giant golden bird cage with a bird inside) tearing right→left along the bottom (26s loop). The generator bakes a fake checkerboard instead of real alpha, so transparency was produced in post: border flood-fill (`/tmp` scripts, Pillow venv) plus a second gray-seeded pass for enclosed regions (cage/window) and a 14px bottom trim. Sprites are `pointer-events-none`, `aria-hidden`, decorative `<img>` (not next/image), and fully hidden under `prefers-reduced-motion`. Movement verified via bounding-box sampling (dragon +51px/2s rightward, chase −92px/2s leftward).

## /marketing creative showcase rebuild (LOCAL ONLY, not deployed) – September 19, 2026

**Stats.** Replaced fabricated performance claims with verifiable promise/fact stats: 10+ years (marketing/data/measurement, F500 work), $0 (growth roadmap delivered before signup, yours to keep), 7 days (kickoff → first campaign live).

**Hero.** Rebuilt as a cinematic image-led composition: new AI editorial photo `public/marketing/hero-cinematic.png` (campaign production desk at golden hour, negative space on the left), full-bleed with a restrained left-weighted ink scrim, white type, same copy and CTAs (Book a demo → /book, report link). Old split hero (`MarketingHeroVisual`) no longer used on the page; component retained in marketing-visuals.tsx.

**Creative showcase.** "Creative that keeps up" tiles rebuilt from CSS UI-card mockups to photographic service tiles — reused the 8 differentiated service photos from `public/marketing/services/` (they were generated to this exact differentiation brief) + new `services/campaigns.png` (campaign operations console, distinct from Paid=media buying and Analytics=measurement). 9 tiles, 3-col desktop bento (two 2x2: Ad creative, Analytics; three 1x2: Short-form, Automation, Email & SMS; four 1x1: Strategy, Ad campaigns, Paid campaigns, Landing pages = filled 3x6 grid, verified). Editorial caption (uppercase label + short phrase) over a bottom scrim replaces the white pill. Motion library: `marketing-showcase-push` (26s slow zoom) and `marketing-showcase-pan` (30s lateral drift), assigned per service; hover = slight crop scale + caption lift. Reduced-motion: all showcase animation off (verified via emulation). Old CreativeVisual CSS-mockup component removed; its keyframes remain in globals.css but are unused.
